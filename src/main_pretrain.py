...
def make_idx_data_term(dataset, args):
    # [(node_id, [(node_id1, ppmi?count?), (node_id2, ppmi?count?), ...]), (), ...]
    data = []
...
def main():
    ...
    parser.add_argument('-d', '--days', type=str,
                        default='1', help='1 7 30 90 180 365 all')
    ...
    parser.add_argument("--dropout", type=float,
                        default=0.5, help="size of testing set")
    parser.add_argument("--log_interval", type=int,
                        default=100, help='step interval for log')
    parser.add_argument("--test_interval", type=int,
                        default=1, help='epoch interval for testing')
    parser.add_argument("--early_stop_epochs", type=int,
                        default=1000, help='epoch interval for early stopping')
    ...
    if os.path.exists(reuse_stored_path):
        ...
        ngram_vocab, ngram_to_id, id_to_gram, pre_train_ngram = loader.g2v_mapping_pretrain(
            list_words, args)
    ...
    if args.neg_sampling:
        ...
        for epoch in range(args.num_epochs):
            ...
            for i in range(num_batches):
                ...
                if steps % args.log_interval == 0:
                    print(
                        'Epoch {0} step {1} - Train loss\u2193:{2:.10}'.format(epoch, steps, np.mean(train_loss)))

            print(
                'Epoch {0} - Train loss\u2193:{1:.10}'.format(epoch, np.mean(train_loss)))
        ...
        for epoch in range(args.num_epochs):
            ...
            print(
                'Epoch {0} - Train loss\u2193:{1:.10}'.format(epoch, train_loss / len(train)))
